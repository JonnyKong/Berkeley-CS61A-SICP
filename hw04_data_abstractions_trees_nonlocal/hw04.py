...
def balanced(m):
    ...
    if is_weight(m):
        ...
        assert is_mobile(m), 'must call balanced() on a mobile'
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        tree_left = totals_tree(end(left(m)))
        tree_right = totals_tree(end(right(m)))
        return tree(label(tree_left) + label(tree_right), [tree_left, tree_right])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pwd):
        nonlocal balance, real_pwd, attempts
        if len(attempts) >= 3:
...
def make_joint(withdraw, old_password, new_password):
    ...
    # If called with incorrect pwd, return immediately
