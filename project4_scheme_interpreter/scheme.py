...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('child frame initialization error')
        while formals != nil:
            # child_frame.bindings[formals.first] = scheme_eval(vals.first, self)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_formals = target.second
        env.bindings[lambda_name] = LambdaProcedure(lambda_formals, expressions.second, env)
        return lambda_name
        # END PROBLEM 10
    else:
